...
void Aes::AES_SetKey(const char *key) {
  AES_set_encrypt_key(reinterpret_cast<const unsigned char *>(key), 128,
                      &enc_key_);
  AES_set_decrypt_key(reinterpret_cast<const unsigned char *>(key), 128,
                      &dec_key_);
}

std::string Aes::AES_Encrypt(const char *input, int length) {
  int encode_len =
      (length + (16 - 1)) & ~(16 - 1); // round up to multiple of 16
  std::string out;
  ...
  for (int i = 0; i < encode_len; i += AES_BLOCK_SIZE) {
    AES_ecb_encrypt(reinterpret_cast<const unsigned char *>(buffer + i),
                    reinterpret_cast<unsigned char *>(out.data() + i),
                    &enc_key_, AES_ENCRYPT);
  }
  free(buffer);
  return out;
}
...
std::string Aes::AES_Decrypt(const unsigned char *input, int length) {
  ...
  for (int i = 0; i < length; i += AES_BLOCK_SIZE) {
    AES_ecb_encrypt(reinterpret_cast<const unsigned char *>(input + i),
                    reinterpret_cast<unsigned char *>(decode_buf + i),
                    &dec_key_, AES_DECRYPT);
  }
